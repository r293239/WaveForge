    ...
    saveGame() {
        const data = {
            ...
            abilities: Abilities.abilities ? Abilities.abilities.map(a => ({
                id: a.id, cooldownRemaining: a.cooldownRemaining || 0, active: a.active || false
            })) : [],
            towers: {
        ...
        try { localStorage.setItem(this.STORAGE_KEY, JSON.stringify(data)); } catch (e) { console.warn('Save failed', e); }
    },

    loadGame() {
        const raw = localStorage.getItem(this.STORAGE_KEY);
        if (!raw) { Messages.show('No save found!'); return false; }
        let data;
        try { data = JSON.parse(raw); } catch (e) { Messages.show('Save corrupted!'); this.clearSave(); return false; }
        ...
        const p = data.player;
        Player.health = p.health; Player.maxHealth = p.maxHealth;
        Player.damageMultiplier = p.damageMultiplier; Player.speedMultiplier = p.speedMultiplier;
        Player.speed = Player.baseSpeed * Player.speedMultiplier;
        Player.lifeSteal = p.lifeSteal; Player.lifeStealRemainder = p.lifeStealRemainder || 0;
        Player.criticalChance = p.criticalChance; Player.goldMultiplier = p.goldMultiplier;
        Player.healthRegen = p.healthRegen; Player.healthRegenPercent = p.healthRegenPercent;
        Player.damageReduction = p.damageReduction; Player.dodgeChance = p.dodgeChance;
        Player.thornsDamage = p.thornsDamage; Player.attackSpeedMultiplier = p.attackSpeedMultiplier;
        Player.reloadSpeedMultiplier = p.reloadSpeedMultiplier;
        Player.firstHitReduction = p.firstHitReduction; Player.firstHitActive = p.firstHitActive;
        Player.guardianAngel = p.guardianAngel; Player.guardianAngelUsed = p.guardianAngelUsed;
        Player.bloodContract = p.bloodContract; Player.bloodContractStacks = p.bloodContractStacks;
        Player.berserkerRing = p.berserkerRing;
        Player.knockback = p.knockback; Player.explosiveKills = p.explosiveKills; Player.goldMagnet = p.goldMagnet;
        Player.facingAngle = p.facingAngle; Player.lastFacingAngle = p.lastFacingAngle;
        ...
        for (let wData of data.weapons) {
            ...
            if (weapon.usesAmmo) { weapon.currentAmmo = wData.currentAmmo ?? weapon.magazineSize; weapon.isReloading = wData.isReloading || false; }
            if (wData.id === 'spear') { weapon.isThrown = wData.isThrown || false; weapon.thrownX = wData.thrownX || 0; weapon.thrownY = wData.thrownY || 0; }
            weapon.poisonDamage = wData.poisonDamage; weapon.poisonDuration = wData.poisonDuration;
            weapon.fireDamage = wData.fireDamage; weapon.fireDuration = wData.fireDuration;
            weapon.bleedDamage = wData.bleedDamage; weapon.bleedDuration = wData.bleedDuration;
            weapon.stunDuration = wData.stunDuration;
            weapon.doubleThrow = wData.doubleThrow; weapon.orbitalMode = wData.orbitalMode;
            weapon.tripleShot = wData.tripleShot; weapon.explosiveShot = wData.explosiveShot;
            weapon.explosiveDamage = wData.explosiveDamage; weapon.explosiveRadius = wData.explosiveRadius;
            weapon.forkLaser = wData.forkLaser; weapon.lightningStrike = wData.lightningStrike;
            weapon.returnSpeed = wData.returnSpeed; weapon.pierceCount = wData.pierceCount;
            weapon.pelletCount = wData.pelletCount; weapon.spreadAngle = wData.spreadAngle;
            weapon.bounceCount = wData.bounceCount; weapon.bounceRange = wData.bounceRange;
        ...
        for (let aData of data.abilities) {
            ...
            if (ability) { ability.cooldownRemaining = aData.cooldownRemaining; ability.active = aData.active; }
        ...
        Game.state = GAME_STATE.SHOP; Game.waveActive = false;
        Shop.generateItems();
        HUD.updateAll(); HUD.updateConsumables();
        Overlays.hideAll();
        Messages.show('Game loaded!');
        return true;
    },

    clearSave() { localStorage.removeItem(this.STORAGE_KEY); }
};
